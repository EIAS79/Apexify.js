...
import type { GIFInputFrame, ImageProperties } from "./lib/index";
...
async function renderAuroraFrame(painter: ApexPainter, t: number): Promise<Buffer> {
...
async function main() {
  ...
  fs.writeFileSync(pngPath, await renderAuroraFrame(painter, 0));
  console.log(
  ...
  for (let i = 0; i < FRAME_COUNT; i++) {
    ...
    gifFrames.push({
      buffer: await renderAuroraFrame(painter, t),
      duration: FRAME_MS,
    });
