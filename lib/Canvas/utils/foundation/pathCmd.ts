import { getPathConstructor } from "./pathUtils";

/**
 * Path command types for building Path2D objects (shared by Path2DCreator and HitDetectionCreator).
 */
...
/**
 * Applies path commands to an existing Path2D-like object.
 */
export function appendPathCommands(path: any, commands: PathCommand[]): void {
...
/**
 * Builds a Path2D instance from command list (single implementation for drawing + hit testing).
 */
export function buildPath2DFromCommands(commands: PathCommand[]): any {
  const PathCtor = getPathConstructor();
  if (!PathCtor) {
    throw new Error('Path2D implementation not found in this runtime');
  }
  const path = new (PathCtor as any)();
