...
export class CanvasCreator {
  ...
  /**
   * Mutates `canvas` to set `width`/`height` when `customBg.inherit` or video frame sizing applies.
   * Same prelude as {@link createCanvas} before allocating the bitmap.
   */
  ...
  /**
   * Core paint pipeline for {@link createCanvas}: backgrounds, pattern, noise, shadow, stroke.
   * Writes into an existing `@napi-rs/canvas` instance of matching width/height.
   */
  ...
  /**
   * Validates + resolves dimensions (mutates `canvas` for `inherit` / video sizing)
   * + paints. Used by {@link createCanvas} and {@link SceneCreator} without a PNG round-trip.
   */
  ...
  /**
   * Paints the same visual pipeline as {@link createCanvas} onto an **existing** canvas instance.
   * Use this from scene composition to avoid an extra PNG encode/decode for the background.
   *
   * @param targetCv - Must already be allocated at the resolved width × height (see below).
   * @param canvas - Config; a shallow copy is used so your object is not mutated except via spread prelude.
   */
