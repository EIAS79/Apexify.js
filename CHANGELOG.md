...
##### Scene — `SceneBuilder` (`scene-builder.ts`)

- **`addLayers(layers)`** — append many layers in paint order; **`addLayer(layer)`** is equivalent to **`addLayers([layer])`**.
- **Stack editing:** **`insertLayer`**, **`insertLayers`**, **`removeLayer`**, **`moveLayer`**, **`replaceLayers`**, **`clearLayers`**, and **`layerCount`** (getter).
- **`toRenderInput()`** — returns a plain **`SceneRenderInput`** with a **copied** **`layers`** array for **`renderScene`**, **`renderSceneToGIF`**, **`renderSceneToVideoFrames`**, caching, or tests.

##### Scene — validation and render options

- **`SceneRenderOptions`** on **`SceneCreator.render`**, **`ApexPainter.renderScene`**, and **`SceneBuilder.render`**: **`validate`** (default **`true`**) and **`maxSurfaceDepth`** (default **64**); set **`{ validate: false }`** only for trusted hot paths.
- **`validateSceneRenderInput`** — exported from the main package entry and the **`scene`** barrel; **`SceneCreate.validateRenderInput`** and **`ApexPainter.validateSceneRenderInput`** delegate to it.

##### Scene — GIF and video helpers

- **`renderSceneToGIF`**: optional **`gif.sceneRender`** — same options as **`renderScene`** for the composed raster step.
- **`renderSceneToVideoFrames`**: optional **`video.sceneRender`** — forwarded when rasterizing the scene PNG before FFmpeg.

### 🔧 Changed

- **`SceneBuilder`** implementation moved from **`scene-creator.ts`** into **`scene-builder.ts`** so the builder API is visible in one file; **`scene-creator.ts`** holds **`SceneCreator`** only. **`scene-renderer.ts`** re-exports both symbols.

### 📚 Documentation

- Scene chapter docs (**`scene/*.mdx`**) updated for **`addLayers`**, stack helpers, **`toRenderInput`**, validation, and **`sceneRender`** on GIF/video, written for the public docs site (no internal repo paths in overviews).
...
##### Charts (`ChartCreator`, `lib/Canvas/utils/Charts`)
...
- **`adjustVolume`**: overall gain (**`volume`** as **percentage**, **100** = unity) and optional **`ranges`** for **per-interval** loudness; ranges may also set **`speed`** and **`pitchSemitones`** (interval pitch/speed via FFmpeg **`rubberband`** when available).

##### ApexPainter
- **`adjustVideoVolume`** now calls **`videoHelpers.adjustVideoVolume`** so **`VideoCreator`** and painter share one implementation.

### 🔧 Improved

##### Charts
- Types and defaults aligned with runtime behavior (titles, axes, legends, and numeric display) for **clearer contracts** and **more predictable** layout across chart kinds.

### 🐛 Fixed

##### Charts
- Various **typing** and **drawing** issues so labels, titles, and values stay **consistent** with options and scale math (including newer scatter / radar / polar-area paths).

### ⚠️ Breaking changes

##### Video volume helper
- Gain for **`adjustVolume`** / **`adjustVideoVolume`** is expressed as a **percentage** (**100** = unchanged). Code that relied on the previous ApexPainter-only **`0–10`** linear **`volume`** argument should use **`100`** for nominal full level (or rescale accordingly).
...
##### Line chart (`linechart.ts`)
- Legend no longer expands canvas with **`extraWidth` / `extraHeight`**; plot rectangle uses **`applyLegendChartAreaInset`** and **`resolveOuterPadding`**.
- Vertical stack uses **`computeChartVerticalStack`** + **`legendInsetGap`** so legend→plot spacing scales; **`top-left`** / **`top-right`** legends use **`legendCornerTopY`**.
- Title Y aligns with **`titleTopInset`** from the stack helper.

##### Bar chart (`barchart.ts`)
- Same inset-only legend canvas, **`resolveOuterPadding`**, **`computeChartVerticalStack`**, and **`vstack.legendInsetGap`** for **`applyLegendChartAreaInset`** (replacing raw **`legend.spacing`** as the sole inset gap).
- Top corner legend positions use **`legendCornerTopY`**.

##### Combo chart
- Same padding/stack/corner-legend behavior as line/bar; baseline preview height accounts for **`axisBottomReserve`** and **`chartAreaTopPre`** after stack math.
- **`legendActive`** guards legend draw path; primary Y spine/ticks use **`yPrimaryTickBase`** when separate from neutral **`axisColor`**.

##### Horizontal bar chart (`horizontalbarchart.ts`)
- Removed legend-driven **`extraWidth` / `extraHeight`**; **`applyLegendChartAreaInset`** + **`resolveOuterPadding`**; **`calculateResponsiveHeight`** uses **`resolveOuterPadding`** for consistent vertical padding.
- Right-edge legends keep full **`chartAreaRight`** (legend past **`axisEndX`**); default **`right`** legend X clamped to stay inside canvas.
- **`originY`** tied to **`chartAreaBottom`** after bottom legend inset.

##### Pie chart (`piechart.ts`)
- Fixed **`width × height`** canvas (no **`extraWidth` / `extraHeight`** stripes); **`resolveOuterPadding`**; connected-legend margin via plot inset; standard legend via **`applyLegendChartAreaInset`**; **`drawStandardLegend`** receives actual canvas dimensions.

##### Defaults
- **`defaultOuterPadding`**: slightly tighter bottom margin (**`base + 13`** vs **`base + 16`**).

### 🐛 Fixed

- **Legend + canvas growth**: Corner/edge legends no longer enlarged the bitmap while leaving **`chartArea*`** tied to the nominal size, which produced large empty margins (notably combo **`top-right`** and similar).
- **Legend flush against plot**: Inset gap now uses **`legendInsetGap`** (canvas-aware) instead of a flat **`10px`** floor alone for **`applyLegendChartAreaInset`** on line/bar/combo.
- **Corner top legends**: No redundant **`chartAreaTop += legendHeight + gap`** for **`top-left`** / **`top-right`**, so the plot gains vertical space; translucent legend may overlap the plot corner by design.
