...
export async function customLines(ctx: SKRSContext2D, options: CustomOptions[]): Promise<void> {
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    const start = inSingleLineSequence && previousEndCoordinates ? previousEndCoordinates : startCoordinates;
    ...
    if (appliedStyle?.gradient) {
      ctx.strokeStyle = createGradientFill(ctx, appliedStyle.gradient, {
        x: start.x,
        y: start.y,
        w: endCoordinates.x - start.x,
        h: endCoordinates.y - start.y,
      });
    } else {
    ...
    if (appliedStyle?.texture) {
      await applyLineTexture(
        ctx,
        appliedStyle.texture,
        appliedStyle.width ?? 1,
        Math.sqrt((endCoordinates.x - start.x) ** 2 + (endCoordinates.y - start.y) ** 2)
      );
    }
...
function applyStroke(
  ctx: SKRSContext2D,
  style: CustomOptions["lineStyle"] | undefined,
  ...
  if (gradient) {
    ctx.strokeStyle = createGradientFill(ctx, gradient, {
      x: start.x,
      y: start.y,
      w: end.x - start.x,
      h: end.y - start.y,
