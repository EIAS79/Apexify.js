...
export function createSmoothPath(
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    for (let s = 0; s <= segmentsPerCurve; s++) {
      const st = s / segmentsPerCurve;
      const point = cubicBezier(p1, { x: cp1x, y: cp1y }, { x: cp2x, y: cp2y }, p2, st);
...
export function createCatmullRomPath(
  ...
  for (let i = 0; i < segments; i++) {
    ...
    for (let s = 0; s <= segmentsPerCurve; s++) {
      ...
      const pt = catmullRom(p0, p1, p2, p3, t, tension);
...
function catmullRom(
  ...
  const x =
    2 * p1.x +
    s *
      ((-p0.x + p2.x) * t + (2 * p0.x - 5 * p1.x + 4 * p2.x - p3.x) * t2 + (-p0.x + 3 * p1.x - 3 * p2.x + p3.x) * t3);

  const y =
    2 * p1.y +
    s *
      ((-p0.y + p2.y) * t + (2 * p0.y - 5 * p1.y + 4 * p2.y - p3.y) * t2 + (-p0.y + 3 * p1.y - 3 * p2.y + p3.y) * t3);
...
export function getPointOnLinePath(
  ...
  for (let i = 0; i < segmentLengths.length; i++) {
    ...
    if (currentLength + segLen >= targetLength) {
      const segmentT = segLen === 0 ? 0 : (targetLength - currentLength) / segLen;
      const dx = points[i + 1]!.x - points[i]!.x;
      const dy = points[i + 1]!.y - points[i]!.y;
      return {
