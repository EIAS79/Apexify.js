...
function integer(value: number, label: string): number {
  if (!Number.isFinite(value) || value <= 0 || !Number.isInteger(value)) throw new ApexifyInputError(`${label} must be a positive integer.`);
  return value;
}

function frameQuality(value: number | undefined, label: string): number {
  const quality = value ?? 2;
  if (!Number.isFinite(quality) || quality < 1 || quality > 31 || !Number.isInteger(quality)) throw new ApexifyInputError(`${label} must be an integer between 1 and 31.`);
  return quality;
}

function outputFormat(value: "jpg" | "png" | undefined): "jpg" | "png" { return value ?? "jpg"; }
...
function inferFormat(outputPath: string, requested?: VideoOutputFormat): VideoOutputFormat {
  ...
  if (ext && inferred && requested && requested !== inferred) throw new ApexifyInputError(`createFromFrames output extension .${ext} does not match requested format ${requested}.`);
  return format;
}

function drawFitted(ctx: ReturnType<typeof getCanvasContext>, image: Awaited<ReturnType<typeof loadImage>>, width: number, height: number, fit: VideoFit): void {
  if (fit === "stretch") { ctx.drawImage(image, 0, 0, width, height); return; }
...
async function assertWritable(pathname: string, overwrite: boolean): Promise<void> {
  if (overwrite) return;
  try { await fs.access(pathname); throw new ApexifyInputError(`Output already exists and overwrite=false: ${pathname}`); }
  catch (error) { if (error instanceof ApexifyInputError) throw error; }
}
...
export class FrameOperations {
  ...
  async extractOne(source: VideoSource, options: { time?: number; frame?: number; width?: number; height?: number; outputFormat?: "jpg" | "png"; quality?: number }, controls: VideoRunControls = {}): Promise<{ buffer: Buffer; canvas: { width: number; height: number } }> {
    return this.runtime.withWorkspace("apexify-frame-", async (workspace) => {
      ...
      let time: number;
      if (options.time !== undefined) time = nonNegativeNumber(options.time, "extractFrame time");
      ...
      if (!Number.isInteger(width) || !Number.isInteger(height) || width <= 0 || height <= 0) throw new ApexifyInputError("extractFrame width and height must be positive integers.");
      assertVideoResourceLimits({ width, height });
      if (width === image.width && height === image.height) return { buffer: raw, canvas: { width, height } };
  ...
  async extractTimes(source: VideoSource, times: number[], options: { outputFormat?: "jpg" | "png"; quality?: number } = {}, controls: VideoRunControls = {}): Promise<Buffer[]> {
  ...
  async extractInterval(source: VideoSource, options: { interval: number; frameSelection?: { start?: number; end?: number }; outputFormat?: "jpg" | "png"; quality?: number; outputDirectory?: string }, controls: VideoRunControls = {}): Promise<Array<{ source: string; isRemote: boolean }>> {
    ...
    return this.runtime.withWorkspace("apexify-interval-", async (workspace) => {
      ...
      if (!Number.isInteger(startFrame) || !Number.isInteger(endFrame) || startFrame < 0 || endFrame < startFrame || endFrame >= totalFrames) throw new ApexifyInputError("extractFrames frameSelection must be an increasing integer range inside the generated frame count.");
      ...
      if (controls.overwrite === false) await assertWritable(path.join(outputDirectory, `frame-${String(1).padStart(6, "0")}.${format}`), false);
      ...
      for (let i = 1; i <= count; i++) {
        ...
        try { await fs.access(framePath); result.push({ source: framePath, isRemote: false }); } catch { break; }
      }
      return result;
    });
  }

  async extractAll(source: VideoSource, options: { outputFormat?: "jpg" | "png"; outputDirectory?: string; quality?: number; prefix?: string; startTime?: number; endTime?: number } = {}, controls: VideoRunControls = {}): Promise<Array<{ source: string; frameNumber: number; time: number }>> {
    ...
    return this.runtime.withWorkspace("apexify-allframes-", async (workspace) => {
      ...
      for (let i = 1; i <= estimatedCount; i++) {
        ...
        try { await fs.access(framePath); } catch { break; }
  ...
  async thumbnail(source: VideoSource, options: { count?: number; grid?: { cols: number; rows: number }; width?: number; height?: number; outputFormat?: "jpg" | "png"; quality?: number }, controls: VideoRunControls = {}): Promise<CanvasResults> {
  ...
  async preview(source: VideoSource, options: { count?: number; outputDirectory?: string; outputFormat?: "jpg" | "png"; quality?: number }, controls: VideoRunControls = {}): Promise<Array<{ source: string; frameNumber: number; time: number }>> {
  ...
  async createFromFrames(options: { frames: VideoSource[]; outputPath: string; fps?: number; format?: VideoOutputFormat; quality?: VideoQuality; bitrate?: number; resolution?: { width?: number; height?: number; fit?: VideoFit } }, controls: VideoRunControls = {}) {
    ...
    return this.runtime.withWorkspace("apexify-frames-video-", async (workspace) => {
      ...
      if (options.bitrate !== undefined) args.push("-b:v", `${options.bitrate}k`); else args.push("-crf", QUALITY_CRF[quality]);
