...
export interface RenderLimits {
  ...
  maxVideoMergeInputs: number;
  maxVideoExtractedFrames: number;
  maxVideoAudioTracks: number;
  maxVideoPipelineLayers: number;
...
export interface CacheRuntimeConfig { enabled: boolean; ttlMs: number; maxEntries: number; maxBytes: number; }
export interface FfmpegRuntimeConfig { ffmpegPath?: string; ffprobePath?: string; processTimeoutMs: number; probeTimeoutMs: number; maxStdoutBytes: number; maxStderrBytes: number; }
...
export const DEFAULT_APEXIFY_RUNTIME_CONFIG: Readonly<ApexifyRuntimeConfig> = Object.freeze({
  ...
  limits: Object.freeze({
    maxCanvasDimension: 16_384, maxTotalPixels: 67_108_864, maxCollectionItems: 2_048, maxBackgroundLayers: 128, maxFiltersPerOperation: 64,
    maxSceneLayers: 2_000, maxSceneTotalPixels: 268_435_456, maxNestedSurfaces: 64, maxSceneDepth: 32, maxSceneImages: 2_000, maxSceneTextLayers: 1_000, maxSceneCharts: 256,
  ...
  cache: Object.freeze({ enabled: true, ttlMs: 5 * 60_000, maxEntries: 128, maxBytes: 128 * 1024 * 1024 }),
  ffmpeg: Object.freeze({ processTimeoutMs: 5 * 60_000, probeTimeoutMs: 5_000, maxStdoutBytes: 10 * 1024 * 1024, maxStderrBytes: 30 * 1024 * 1024 }),
  temp: Object.freeze({ retainFiles: false }),
  diagnostics: Object.freeze({}),
});

function finitePositive(name: string, value: number): number { if (!Number.isFinite(value) || value <= 0) throw new ApexifyConfigError(`${name} must be a finite positive number.`); return value; }
function finitePositiveInteger(name: string, value: number): number { finitePositive(name, value); if (!Number.isInteger(value)) throw new ApexifyConfigError(`${name} must be an integer.`); return value; }
function finiteNonNegative(name: string, value: number): number { if (!Number.isFinite(value) || value < 0) throw new ApexifyConfigError(`${name} must be a finite non-negative number.`); return value; }
function optionalNonEmptyString(name: string, value: string | undefined): string | undefined { if (value === undefined) return undefined; if (typeof value !== "string" || value.length === 0 || value.includes("\0")) throw new ApexifyConfigError(`${name} must be a non-empty string without NUL bytes when provided.`); return value; }
...
export function resolveApexifyRuntimeConfig(input: ApexifyRuntimeConfigInput = {}): Readonly<ApexifyRuntimeConfig> {
  ...
  if (network.allowedProtocols.length === 0 || network.allowedProtocols.some((p) => p !== "http:" && p !== "https:")) throw new ApexifyConfigError("network.allowedProtocols may contain only http: and https: and cannot be empty.");
  if (network.trustedNetworkAccess && network.allowedHosts.length === 0) throw new ApexifyConfigError("network.trustedNetworkAccess requires at least one explicit network.allowedHosts entry.");
  ...
  ffmpeg.processTimeoutMs = finitePositive("ffmpeg.processTimeoutMs", ffmpeg.processTimeoutMs); ffmpeg.probeTimeoutMs = finitePositive("ffmpeg.probeTimeoutMs", ffmpeg.probeTimeoutMs); ffmpeg.maxStdoutBytes = finitePositive("ffmpeg.maxStdoutBytes", ffmpeg.maxStdoutBytes); ffmpeg.maxStderrBytes = finitePositive("ffmpeg.maxStderrBytes", ffmpeg.maxStderrBytes);
  ...
  if (diagnostics.handler !== undefined && typeof diagnostics.handler !== "function") throw new ApexifyConfigError("diagnostics.handler must be a function when provided.");
  return Object.freeze({ network: Object.freeze(network), limits: Object.freeze(limits), cache: Object.freeze(cache), ffmpeg: Object.freeze(ffmpeg), temp: Object.freeze(temp), diagnostics: Object.freeze(diagnostics) });
}
...
export function setDefaultApexifyRuntimeConfig(input: ApexifyRuntimeConfigInput = {}): Readonly<ApexifyRuntimeConfig> { defaultRuntimeConfig = resolveApexifyRuntimeConfig(input); return defaultRuntimeConfig; }
export function resetDefaultApexifyRuntimeConfig(): Readonly<ApexifyRuntimeConfig> { defaultRuntimeConfig = resolveApexifyRuntimeConfig(); return defaultRuntimeConfig; }

/** Backward-compatible process-wide configuration merge. */
export function configureApexifyRuntime(input: ApexifyRuntimeConfigInput): Readonly<ApexifyRuntimeConfig> {
  ...
  return setDefaultApexifyRuntimeConfig({
    network: { ...current.network, ...input.network }, limits: { ...current.limits, ...input.limits }, cache: { ...current.cache, ...input.cache },
    ffmpeg: { ...current.ffmpeg, ...input.ffmpeg }, temp: { ...current.temp, ...input.temp }, diagnostics: { ...current.diagnostics, ...input.diagnostics },
  });
}
export function resetApexifyRuntimeConfig(): Readonly<ApexifyRuntimeConfig> { return resetDefaultApexifyRuntimeConfig(); }
