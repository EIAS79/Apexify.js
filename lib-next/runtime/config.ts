...
export function resolveApexifyRuntimeConfig(input: ApexifyRuntimeConfigInput = {}): Readonly<ApexifyRuntimeConfig> {
  ...
  if (limits.maxVideoAudioTracks > limits.maxVideoOverlays) {
    throw new ApexifyConfigError("limits.maxVideoAudioTracks must be <= limits.maxVideoOverlays.");
  }
  if (limits.maxVideoMergeInputs > limits.maxCollectionItems) {
    throw new ApexifyConfigError("limits.maxVideoMergeInputs must be <= limits.maxCollectionItems.");
  }
  if (limits.maxVideoPipelineLayers > limits.maxCollectionItems) {
    throw new ApexifyConfigError("limits.maxVideoPipelineLayers must be <= limits.maxCollectionItems.");
  }
  if (limits.maxVideoExtractedFrames > limits.maxCollectionItems) {
    throw new ApexifyConfigError("limits.maxVideoExtractedFrames must be <= limits.maxCollectionItems.");
  }
...
export function setDefaultApexifyRuntimeConfig(input: ApexifyRuntimeConfigInput = {}): Readonly<ApexifyRuntimeConfig> {
  defaultRuntimeConfig = resolveApexifyRuntimeConfig(input);
  return defaultRuntimeConfig;
}
export function resetDefaultApexifyRuntimeConfig(): Readonly<ApexifyRuntimeConfig> {
