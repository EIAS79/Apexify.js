...
import { paintChartCanvasBackground } from "../helpers/chartBackground";
import { normalizeLegendPosition, applyLegendChartAreaInset } from "../helpers/legendPlacement";
...
async function renderEnhancedText(
  ...
  if (style?.fontPath && style?.fontName) {
    try {
      ...
      emitDiagnostic({ level: "warn", code: "APEXIFY_PIECHART_WARN", message: "A non-fatal Apexify warn diagnostic was emitted by lib-next/chart/impl/piechart.ts." });
    }
  }
